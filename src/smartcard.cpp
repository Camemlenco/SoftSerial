...
/**
  * @brief  Envia um comando APDU Raw (comprimento explícito) e recebe a resposta via protocolo T=1.
  * * Adaptação da função "T1_APDU".
  * @param  p_t1: ponteiro para a estrutura de contexto do protocolo smartcard.
  * @param  nad: valor do nad (Node Address).
  * @param  p_apdu_c: ponteiro para o buffer de comando ADPU.
  * @param  apdu_c_len: O comprimento total e exato do comando APDU a ser enviado.
  * @param  p_apdu_r: ponteiro para o buffer de resposta ADPU.
  ...
  */
int32_t T1_APDU_Raw(SCProtocol_t * p_t1, uint8_t nad, SC_APDU_t *p_apdu_c, uint32_t apdu_c_len, SC_APDU_t *p_apdu_r)
{
  ...
  // LINHA APAGADA: apdu_c_len = p_apdu_c->APDU_S.Body.LC + 5;
  // O comprimento `apdu_c_len` agora é passado como um argumento da função.

  p_t1->state = T1_SENDING;
  retries = p_t1->retries;
  resyncs = 10;

  // O resto da função permanece exatamente igual ao T1_APDU original
